 ...
 */
class AnswerTypeSliderFix(context: QuestionnaireActivity?, questionnaire: Questionnaire?, qParent: AnswerLayout?, nQuestionId: Int, questionText: QuestionText?, immersive: Boolean) : AnswerType(context, questionnaire, qParent, nQuestionId) {
    //public final AnswerLayout parent;
    private val mListOfIds: MutableList<Int?> = ArrayList()
    override val mContext: QuestionnaireActivity?
    ...
    private val mSliderContainer: RelativeLayout?
    ...
    private val mQuestionText: QuestionText?
    ...
    override fun addClickListener() {
        ...
        tvTemp.post { // Handles default id if existent
            if (mDefaultAnswer == -1) {
                setProgressItem(((mListOfAnswers.size - 1) / 2.0f).toInt())
                mQuestionnaire!!.addIdToEvaluationList(mQuestionId,
                        mListOfAnswers[mListOfAnswers.size / 2].id)
            } else {
                setProgressItem(mDefaultAnswer)
                mQuestionnaire!!.addIdToEvaluationList(mQuestionId,
                        mListOfAnswers[mDefaultAnswer].id)
            }
        }
    ...
    private fun rescaleSliderFinal(motionEvent: MotionEvent?): Boolean {
        ...
        var nItem = mapValuesToItems(nValueSelected.toFloat())
        nItem = clipItemsToRange(nItem)
        try {
            setProgressItem(nItem)
            mQuestionnaire!!.removeQuestionIdFromEvaluationList(mQuestionId)
            mQuestionnaire.addIdToEvaluationList(mQuestionId, mListOfAnswers[nItem].id)
    ...
    private fun clipItemsToRange(item: Int): Int {
        var item = item
    ...
    private fun mapValuesToItems(inVal: Float): Int {
        val nTotalHeight = mRemainView!!.height
        val topPadHeight = (mContext!!.resources.getDimension(R.dimen.toolBarHeightWithPadding) +
                mContext.resources.getDimension(R.dimen.progressBarHeight) + mContext.resources.getDimension(R.dimen.answerLayoutPadding_Bottom).toInt()).toInt()
        return (inVal - topPadHeight - mQuestionText!!.questionHeight/ (nTotalHeight / mListOfAnswers.size)).toInt()
    }

    // Set progress/slider according to number of selected item (counting from 0)
    fun setProgressItem(numItem: Int) {
    ...
    companion object {
        var LOG_STRING: String? = "AnswerTypeSliderFix"
    ...
    init {
        ...
        mSliderContainer = mHorizontalContainer.findViewById<View?>(
